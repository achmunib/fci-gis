...
<script>
   ...
   var baseMaps = {
    ...
    let multipolygongroup = L.layerGroup().addTo(map)
        L.polygon([<?=$data->polygon?>], multiPolygonOptions)
            .bindPopup("ID PEMDA : <?=$data->id_pemda?><br>"+
                ...
                "UPT : <?=$data->upt?><br>"+
                "<a href='{{ route('outlets.create') }}?id_pemda=' + id_pemda + ''>Data Detail</a>")
            .addTo(multipolygongroup);
        
        overlayMaps['{!! $data->nama_opd !!}'] = multipolygongroup;
